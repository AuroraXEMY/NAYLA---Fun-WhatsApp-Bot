...
const groupMessageBuffers = new Map(); // groupJid -> [{sender, text, ts}], capped to MOVIE_MODE_BUFFER_CAP
...
const MOVIE_MODE_BUFFER_CAP = 120;  // ~a few KB per active group — trivial memory cost
...
function getGroupConfig(jid) {
  if (!groupConfigCache.has(jid)) {
    groupConfigCache.set(jid, { locked: false, lastRecapDate: null, dirty: false });
  }
...
async function persistGroupConfig(jid) {
  ...
  try {
    await GroupConfig.findOneAndUpdate(
      { jid },
      { locked: cfg.locked, lastRecapDate: cfg.lastRecapDate },
      { upsert: true }
    );
  } catch (err) {
...
async function loadGroupConfigsFromMongo() {
  ...
  try {
    ...
    for (const doc of all) {
      groupConfigCache.set(doc.jid, { locked: !!doc.locked, lastRecapDate: doc.lastRecapDate || null, dirty: false });
    }
...
function bufferGroupMessage(jid, sender, text) {
  ...
  if (buf.length > MOVIE_MODE_BUFFER_CAP) buf.shift(); // hard cap — oldest messages roll off
}
...
async function generateAIChatReply(senderJid, sender, question) {
  if (!process.env.GROQ_API_KEY) {
  ...
  const callPromise = (async () => {
    const systemPrompt = `You are ${BOT_CONFIG.name}, a WhatsApp group companion with a "${BOT_CONFIG.vibe}" personality (cool, laid-back, uses 😎 🌴 slang where natural).
...
async function startBot() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      if (isGroup) bufferGroupMessage(jid, sender, text);
      ...
      if (shouldChatReply && cooledDown) {
        ...
        const aiResult = await generateAIChatReply(senderJid, sender, text);
        try {
