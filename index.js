...
function getGroupConfig(jid) {
  if (!groupConfigCache.has(jid)) {
    groupConfigCache.set(jid, { locked: false, lastRecapDate: null, mood: "cool", muted: false, ignoredUsers: [], dirty: false });
  }
...
async function handleCommand(sock, jid, senderJid, sender, text, msg) {
  ...
  try {
    ...
    if (jid.endsWith("@g.us") && (cmd === ".ignore" || cmd.startsWith(".ignore "))) {
      ...
      const cfg = getGroupConfig(jid);
      if (!cfg.ignoredUsers.includes(target)) cfg.ignoredUsers.push(target);
      cfg.dirty = true;
      await persistGroupConfig(jid);
    ...
    if (jid.endsWith("@g.us") && (cmd === ".undoignore" || cmd.startsWith(".undoignore "))) {
      ...
      if (arg === "all") {
        ...
        cfg.ignoredUsers = cfg.ignoredUsers.filter(u => u !== target);
      }
      cfg.dirty = true;
      await persistGroupConfig(jid);
...
function applyEmojiPolicy(jid, text) {
  const hasEmoji = EMOJI_REGEX.test(text);
  let finalText = text;
  if (hasEmoji && shouldStripEmoji(jid, text)) {
...
async function sendLikeAHuman(sock, jid, msg, rawText) {
  const text = applyEmojiPolicy(jid, rawText);
...
async function startBot() {
  ...
  sock.ev.on("messages.upsert", async (m) => {
    ...
    for (const msg of m.messages) {
      ...
      // .mute / .ignore — both checked here, AFTER command routing above (so
      // .unmute / .undoignore still work while active), but BEFORE anything
      // else — a muted group or an ignored user gets zero reaction, zero
      // comment, zero reply, exactly as requested. No AI cost either way.
      if (isGroup) {
        const groupCfg = getGroupConfig(jid);
        if (groupCfg.muted) continue;
        if (groupCfg.ignoredUsers?.includes(senderJid)) continue;
      }
