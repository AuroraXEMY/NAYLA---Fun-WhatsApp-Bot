...
Respond ONLY with raw JSON matching this schema:
{
  ...
  try {
    ...
    const { category, detail } = describeGeminiError(err);
...
// --- Classify Gemini/API failures into a clear, human-readable reason ---
...
function describeGeminiError(err) {
  ...
  if (msg.includes("timed out")) {
    return { category: "TIMEOUT", detail: "Gemini call exceeded the timeout window.", userText: "⏱️ My AI brain took too long to respond. Try again in a sec!" };
  }
  if (msg.includes("api key not valid") || msg.includes("api_key_invalid") || msg.includes("401") || msg.includes("permission_denied") || msg.includes("403")) {
    return { category: "AUTH", detail: "GEMINI_API_KEY is missing, invalid, or lacks permission.", userText: "🔑 My AI connection is misconfigured (invalid/missing API key) — my developer needs to check GEMINI_API_KEY." };
  }
  if (msg.includes("econnrefused") || msg.includes("enotfound") || msg.includes("fetch failed") || msg.includes("network") || msg.includes("eai_again")) {
    return { category: "CONNECTION", detail: "Could not reach Google's Gemini API servers (connection refused/DNS/network).", userText: "🌐 I couldn't reach the AI servers (connection refused). Try again shortly!" };
  }
  if (msg.includes("429") || msg.includes("resource_exhausted") || msg.includes("rate limit") || msg.includes("quota")) {
    return { category: "RATE_LIMIT", detail: "Gemini API quota or rate limit exceeded.", userText: "🚦 I'm being rate-limited by the AI provider right now. Give me a minute!" };
  }
  if (msg.includes("404") || msg.includes("not found")) {
    return { category: "BAD_MODEL", detail: "The configured Gemini model name was not found/is invalid.", userText: "❓ My AI model config looks wrong — my developer needs to check the model name." };
  }
...
async function generateAIChatReply(sender, question) {
  if (!process.env.GEMINI_API_KEY) {
    ...
    return { success: false, message: "🔑 My AI connection is misconfigured (missing API key) — my developer needs to set GEMINI_API_KEY." };
  }

  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new Error("Gemini chat call timed out after 15 seconds")), 15000);
  });

  const callPromise = (async () => {
    const prompt = `You are ${BOT_CONFIG.name}, a WhatsApp group companion with a "${BOT_CONFIG.vibe}" personality (cool, laid-back, uses 😎 🌴 slang where natural).
Someone just tagged you directly and said:
"${sender}: ${question}"

Reply naturally and helpfully, in character, in 1-4 sentences. Do not mention you are an AI model unless directly asked.`;
  ...
  try {
    ...
    const { category, detail, userText } = describeGeminiError(err);
