...
function applyEmojiPolicy(jid, text, allowEmoji = false) {
  ...
  // 🛡️ FIX: If stripping left the text empty, keep the original
...
async function sendLikeAHuman(sock, jid, msg, rawText, allowEmoji = false) {
  let text = applyEmojiPolicy(jid, rawText, allowEmoji);

  // 🛡️ FIX: ensure we never send an empty message (which can hang DMs)
  if (!text || !text.trim()) {
    text = "🤖 (I had a reply, but it disappeared!)";
  }
  ...
  try {
    ...
    console.error(`❌ sendLikeAHuman: failed to send message to ${jid}:`, sendErr.message);
    // Rethrow so the outer handler can log/fall back
    throw sendErr;
  }
}
